...
# -----------------------------------------------------------------------------
# Paths
PATH <- './sparse/results/pve'
GRAPHS <- './graphs/sparse/pve'
...
plot_eigenvalues <- function(idx) {
    ...
    saveRDS(pct_explained, file = paste0('./sparse/results/pct_explained/', name))
    
    name <- paste0(
        '/npc_estim_N_', eigenvalues$N, '_M_', eigenvalues$M,
        '_univ_', eigenvalues$NPC, '.rds'
    )
    saveRDS(NPC_estim, file = paste0('./sparse/results/npc_estim/', name))
}
